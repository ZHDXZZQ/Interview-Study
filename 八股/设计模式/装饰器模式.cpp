...
public:
    ...
    std::string get_equipment() override {  // 装饰英雄的武器
        return Decorator::get_equipment() + " 刀， ";
    }
};
...
public:
    ...
    std::string get_equipment() override {  // 装饰英雄的武器
        return Decorator::get_equipment() + " 枪， ";
    }
};

int main() {
    Hero* iron_man = new Hero("iron man");
    iron_man = new Knife(iron_man);
    std::cout << iron_man->get_equipment() << std::endl;

    iron_man = new Gun(iron_man);
    std::cout << iron_man->get_equipment() << std::endl;;
